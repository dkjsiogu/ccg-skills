---
description: '多模型调试：Codex 后端诊断 + Copilot 前端诊断，交叉验证定位问题'
---
...
| 模型 | 提示词 |
|------|--------|
| Codex | `/home/dkjsiogu/.claude/.ccg/prompts/codex/debugger.md` |
| Copilot | `/home/dkjsiogu/.claude/.ccg/prompts/copilot/debugger.md` |
...
1. **Codex 后端诊断**：`Bash({ command: "...--backend codex...", run_in_background: true })`
   - ROLE_FILE: `/home/dkjsiogu/.claude/.ccg/prompts/codex/debugger.md`
   - OUTPUT：诊断假设（按可能性排序），每个假设包含原因、证据、修复建议

2. **Copilot 前端诊断**：`Bash({ command: "...--backend gemini...", run_in_background: true })`
   - ROLE_FILE: `/home/dkjsiogu/.claude/.ccg/prompts/copilot/debugger.md`
   - OUTPUT：诊断假设（按可能性排序），每个假设包含原因、证据、修复建议
...
1. **用户确认** – 修复前必须获得确认
2. **信任规则** – 后端问题以 Codex 为准，前端问题以 Copilot 为准
3. 外部模型对文件系统**零写入权限**
