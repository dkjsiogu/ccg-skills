---
description: '多模型测试生成：智能路由 Codex 后端测试 / Copilot 前端测试'
---
...
- 测试目标：$ARGUMENTS
- 智能路由：后端 → Codex，前端 → Copilot，全栈 → 并行
- 遵循项目现有测试框架和风格
...
| 模型 | 提示词 |
|------|--------|
| Codex | `/home/dkjsiogu/.claude/.ccg/prompts/codex/tester.md` |
| Copilot | `/home/dkjsiogu/.claude/.ccg/prompts/copilot/tester.md` |
...
- **后端代码** → `Bash({ command: "...--backend codex...", run_in_background: false })`
  - ROLE_FILE: `/home/dkjsiogu/.claude/.ccg/prompts/codex/tester.md`
- **前端代码** → `Bash({ command: "...--backend gemini...", run_in_background: false })`
  - ROLE_FILE: `/home/dkjsiogu/.claude/.ccg/prompts/copilot/tester.md`
- **全栈代码** → 并行调用两者：
...
1. **测试行为，不测试实现** – 关注输入输出
2. **智能路由** – 后端测试用 Codex，前端测试用 Copilot
