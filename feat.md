...
**如果 ace-tool MCP 可用**，调用 `mcp__ace-tool__enhance_prompt`，**用增强结果替代原始 $ARGUMENTS，后续调用 Codex/Copilot 时传入增强后的需求**
...
```
执行 agent: /home/dkjsiogu/.claude/agents/ccg/ui-ux-designer.md
输入: 项目上下文 + 用户需求 + 技术栈
输出: UI/UX 设计方案
```
...
```
执行 agent: /home/dkjsiogu/.claude/agents/ccg/planner.md
...
- **前端任务**：调用 Copilot，使用实施提示词
- **后端任务**：调用 Codex，使用实施提示词
- **全栈任务**：并行调用 Codex + Copilot（`run_in_background: true`），用 `TaskOutput` 等待结果
