---
description: '多模型技术分析（并行执行）：Codex 后端视角 + Copilot 前端视角，交叉验证后综合见解'
---
...
| 模型 | 提示词 |
|------|--------|
| Codex | `/home/dkjsiogu/.claude/.ccg/prompts/codex/analyzer.md` |
| Copilot | `/home/dkjsiogu/.claude/.ccg/prompts/copilot/analyzer.md` |
...
2. 识别：
   ...
   - **分歧点**（需权衡）
   - **互补见解**（各自领域洞察）
3. 按信任规则权衡：后端以 Codex 为准，前端以 Copilot 为准
...
2. **信任规则** – 后端以 Codex 为准，前端以 Copilot 为准
3. 外部模型对文件系统**零写入权限**
